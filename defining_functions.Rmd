...
avg(x=dat, arithmetic=TRUE)  # best coding practice bc each parameter is very explicit
avg(x=dat, arithmetic=FALSE)  # reversed the order of the parameters without explicitly naming the parameters
```
...
In the last example, since the parameters weren't explicitly named, the `avg()` function assumed that `x=FALSE` and attempted to take the average of `FALSE`, resulting in unexpected behavior. Good programmers who write functions to share with others will deliberately code checks and error reporting into the functions that they write. For example, at the beginning of the `avg()` function, we could check to see if `x` is a vector and print out an error message if it's not:

```{r}
# Calculate arithmetic mean by default, or calculate geometric mean if the parameter 'arithmetic' is set to FALSE
avg = function(x, arithmetic = TRUE){
  if(!is.numeric(x)) {return("Error: function failed. x must be a numeric")}
  n = length(x)
  ifelse(arithmetic, sum(x)/n, prod(x)^(1/n))
}
